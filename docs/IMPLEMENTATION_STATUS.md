...
El 21 de julio una entrega de prueba de Portoviejo fallo tres intentos con `invalid_model_output`. La reproduccion local con los mismos datos capturo el error real: `ModelBehaviorError: Invalid output type: Unterminated string in JSON`, porque `maxTokens` (1800 por especialista, 5000 en la sintesis) tambien descuenta los tokens de razonamiento del modelo y truncaba el JSON estructurado. Los presupuestos subieron a 8000 y 16000; la reproduccion local completo con exito y el cambio esta desplegado. Los analisis previos que completaron lo hicieron porque su razonamiento consumio menos presupuesto; el fallo era probabilistico, no de infraestructura.
...
## Entrega de creditos por difusion (desplegada)
