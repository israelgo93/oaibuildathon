...
- Public, judge, and mentor reads select the most recent event by `starts_at`. The admin dashboard returns every event; the local admin UI adds an event picker, event creation (`create_event` with optional rubric copy), per-event scoping of teams/challenges/submissions/assignments/results, and random judge/mentor distribution. Verify deployment state before assuming production behavior.
